...
開発サーバーは通常 `http://127.0.0.1:5173/` で起動します。すでに使用中の場合はViteが次の空きポートを表示します。
...
- `hero-visual.png`: ヒーロー用の占い師風キャラクタービジュアル
- `hero-bg.png`: ヒーロー背景用の淡い占い背景
...
「絶対当たる」「必ず復縁できる」などの断定表現は使わず、恋を整理するためのヒントとして安心して読める文言にしています。
